  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const pending = res.items.filter(b =>
        ...
        b.batchStatus === BatchStatus.ScanningInProgress ||
        b.batchStatus === BatchStatus.ScanningPending
      );
      setBatches(pending);
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '200px' }}>
        <div style={{ fontSize: 'var(--text-sm)', color: 'var(--fg-muted)' }}>Loading scanning queue...</div>
      </div>
    );
  }
