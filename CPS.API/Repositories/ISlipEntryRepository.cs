...
{
    Task<SlipEntry?> GetByIdAsync(int slipEntryId);
    Task<List<SlipEntry>> GetByBatchAsync(long batchId);
    Task<bool> SlipNoExistsAsync(long batchId, string slipNo, int? excludeId = null);
    ...
    Task<List<SlipScan>> GetSlipScansByEntryAsync(int slipEntryId);
    ...
    Task<List<ChequeItem>> GetChequeItemsBySlipAsync(int slipEntryId);
    Task<ChequeItem> CreateChequeItemAsync(ChequeItem item);
    Task UpdateChequeItemAsync(ChequeItem item);
    Task<int> GetNextBatchSeqNoAsync(long batchId);
    Task<int> GetNextChqSeqAsync(int slipEntryId);
